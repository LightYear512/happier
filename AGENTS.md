...
- Fork release work: for release validation, promotion, GitHub release, npm, Docker, Expo, deploy, or other publication work in this fork, never rely on hardcoded upstream repository values such as `happier-dev/happier`. Default to the current fork repository `LightYear512/happier` for this checkout, or resolve the repository from `git remote get-url origin` when the local origin points elsewhere. Pass the resolved repository explicitly to release commands through flags such as `--repository <owner>/<repo>` or environment such as `GH_REPO`/`GITHUB_REPOSITORY` when tooling reads them. Run the canonical dry run first, summarize planned side effects, and ask for explicit approval before any non-dry-run release, publish, or deploy command.
