...
test('pipeline CLI preview release dry-run does not require a main branch in fork repos', async () => {
  const stub = createReleaseCliDryRunEnv(process.env, { missingRemoteBranches: ['main'] });
  try {
    const out = execFileSync(
      process.execPath,
      [
    ...
    assert.match(out, /\[pipeline\] release: fetching origin dev\/preview for plan/);
