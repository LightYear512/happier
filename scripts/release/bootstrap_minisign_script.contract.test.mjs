...
test('bootstrap-minisign script prefers Windows process architecture over host uname architecture', async () => {
  const root = await mkdtemp(join(tmpdir(), 'bootstrap-minisign-win-process-arch-'));
  const binDir = join(root, 'bin');
  const runnerTemp = join(root, 'runner-temp');
  const fixtureDir = join(root, 'fixture');
  await mkdir(binDir, { recursive: true });
