...
if (method === 'GET') {
  const outcome = process.env.GH_STUB_GET_OUTCOME ?? 'success';
  if (outcome === '404') {
...
function runPromotionWithStub(tmpDir, getOutcome) {
  ...
  const env = {
    ...
    GH_STUB_GET_OUTCOME: getOutcome,
  };
  const args = [
...
test('promote deploy branch sends one PATCH and no POST when the ref exists', () => {
  ...
  assert.deepEqual(calls.map((args) => args.includes('-X') ? args[args.indexOf('-X') + 1] : 'GET'), ['GET', 'PATCH']);
...
test('promote deploy branch creates the ref only after an explicit GET 404', () => {
  const tmpDir = fs.mkdtempSync(resolve(os.tmpdir(), 'happier-promote-missing-'));
  const { calls, error } = runPromotionWithStub(tmpDir, '404');
  assert.equal(error, undefined);
  const patchCalls = calls.filter((args) => args.includes('PATCH'));
  const postCalls = calls.filter((args) => args.includes('POST'));
  assert.deepEqual(calls.map((args) => args.includes('-X') ? args[args.indexOf('-X') + 1] : 'GET'), ['GET', 'POST']);
  assert.equal(patchCalls.length, 0, `unexpected PATCH: ${JSON.stringify(calls)}`);
  assert.equal(postCalls.length, 1, `expected one POST, got ${JSON.stringify(calls)}`);
});

test('promote deploy branch fails closed without mutation for non-404 GET failures', () => {
  ...
  const patchCalls = calls.filter((args) => args.includes('PATCH'));
  const postCalls = calls.filter((args) => args.includes('POST'));
  assert.deepEqual(calls.map((args) => args.includes('-X') ? args[args.indexOf('-X') + 1] : 'GET'), ['GET']);
  assert.equal(patchCalls.length, 0, `unexpected PATCH: ${JSON.stringify(calls)}`);
  assert.equal(postCalls.length, 0, `unexpected POST: ${JSON.stringify(calls)}`);
});
