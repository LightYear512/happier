...
test('expo native-build supports local mode and writes build metadata json', () => {
  ...
  writeExecutable(
    ...
    [
      ...
      '  echo \'{"hash":"fp-local-build-test","sources":[],"fileHookTransformConfig":{}}\';',
      '  exit 0',
      'fi',
...
test('expo native-build runs local builds non-interactively in CI', () => {
  ...
  writeExecutable(
    ...
    [
      ...
      'echo "NPX $*"',
      'if printf "%s\\n" "$*" | grep -q "fingerprint:generate"; then',
      '  echo \'{"hash":"fp-local-ci-test","sources":[],"fileHookTransformConfig":{}}\';',
      '  exit 0',
      'fi',
      // Simulate `eas build --local --output <path>` by creating the output file.
      'out=""',
...
test('expo native-build allows interactive local builds when PIPELINE_INTERACTIVE=1', () => {
  ...
  writeExecutable(
    ...
    [
      ...
      'echo "NPX $*"',
      'if printf "%s\\n" "$*" | grep -q "fingerprint:generate"; then',
      '  echo \'{"hash":"fp-local-interactive-test","sources":[],"fileHookTransformConfig":{}}\';',
      '  exit 0',
      'fi',
      // Simulate `eas build --local --output <path>` by creating the output file.
      'out=""',
