...
test('tests workflow keeps slow CI jobs above the observed timeout floor', async () => {
  ...
  assert.match(
    uiE2eJob,
    /name:\s*UI E2E \(Playwright\)[\s\S]*?timeout-minutes:\s*90\b/,
  ...
  assert.match(
    ...
    'UI E2E should stay split across six shards so release-critical Playwright lanes finish before the job timeout',
  );
