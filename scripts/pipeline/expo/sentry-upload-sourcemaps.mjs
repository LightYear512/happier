...
/**
 * Resolve the exact uploader owned by the UI workspace rather than downloading
 * and executing a mutable package while the Sentry token is present.
 *
 * @param {string} uiDir
 * @returns {string}
 */
...
export function maybeUploadSentryExpoSourceMaps(input) {
  ...
  const { cmd, args } = buildSentryExpoUploadCommand({
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (!uiDir || !distDir) {
    throw new Error('Sentry source-map upload requires --ui-dir and --dist-dir');
  }
