...
export async function createSignedReleaseAssetEnvelope(params) {
  ...
  const assetNames = [...new Set(params.assetNames.map(requireFlatAssetName))].sort();
  ...
  const stats = await Promise.all(assetNames.map(async (name) => {
    ...
    const info = await lstat(assetPath).catch(() => null);
    if (!info?.isFile() || info.isSymbolicLink()) {
      fail(`declared release asset is missing or not a regular file: ${name}`);
    }
