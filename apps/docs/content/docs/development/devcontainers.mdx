...
## Optional: start the local server on boot

Set `HAPPIER_DEV_BOX_AUTOSTART_SERVER=1` to start the local Happier server from the mounted
workspace before the container shell starts:
...
By default, the entrypoint uses `/workspace/happier`, `HAPPIER_STACK_SERVER_PORT=4101`, and
`HAPPIER_SERVER_URL=http://127.0.0.1:4101`. Override `HAPPIER_DEV_BOX_WORKSPACE`,
`HAPPIER_STACK_SERVER_PORT`, `HAPPIER_DEV_BOX_SERVER_FLAVOR`, or `HAPPIER_DEV_BOX_SERVER_BIND`
when your container layout needs different values.
