...
# OAuth return redirect back to the client (required unless you are using https://proxyapi.layaair.com)
...
Required **unless** you are using the hosted client at `https://proxyapi.layaair.com`:
- `HAPPIER_WEBAPP_URL` (default `https://proxyapi.layaair.com`)
...
- `HAPPIER_WEBAPP_URL` / `HAPPIER_WEBAPP_OAUTH_RETURN_URL_BASE` are only “optional” if you are using the hosted client at `https://proxyapi.layaair.com`.
...
- Default return base: `HAPPIER_WEBAPP_URL` (`https://proxyapi.layaair.com`)
...
Troubleshooting:
- If you get redirected to `https://proxyapi.layaair.com` even after setting `HAPPIER_WEBAPP_URL`, the server likely rejected your configured return URL as unsafe. For local web dev, use `http://localhost:PORT` or `http://*.localhost:PORT` (or use `https://`).
