...
The same terminal foundation is also reused for **provider login flows** from provider settings and for supported profile-specific native CLI login flows.
...
- Provision a supported profile's native CLI login on a selected machine without leaving Happier.
...
Provider login flows are documented in [Provider authentication](/docs/features/provider-authentication).

Profile-specific native CLI login uses the same PTY transport, but the daemon prepares an isolated profile auth session first so credentials land under the selected machine's Happier home instead of the provider's default CLI account directory. The detailed architecture is documented in [Native CLI profile authentication](/docs/architecture/native-cli-profile-auth).
...
For daemon setup and lifecycle commands, see [Daemon](/docs/clients/daemon).
