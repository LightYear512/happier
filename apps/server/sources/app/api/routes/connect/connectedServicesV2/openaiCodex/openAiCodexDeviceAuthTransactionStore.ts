...
type TransactionSecret = Readonly<{
    ...
    completionBundleB64Url: string | null;
}>;
...
export type OpenAiCodexDeviceAuthTransactionWork =
    ...
    | Readonly<{ kind: "completed"; bundleB64Url: string }>
    | Readonly<{
        kind: "provider_poll";
...
function decryptSecret(transactionId: string, secretEnc: string): TransactionSecret | null {
    try {
        ...
        if (
            ...
            || (
                parsed.completionBundleB64Url !== undefined
                && parsed.completionBundleB64Url !== null
                && typeof parsed.completionBundleB64Url !== "string"
            )
        ) {
            return null;
        }
        return {
            ...
            completionBundleB64Url: parsed.completionBundleB64Url ?? null,
        };
    } catch {
        return null;
    }
...
export async function claimOpenAiCodexDeviceAuthTransactionWork(input: Readonly<{
    ...
    return await inTx(async (tx) => {
        ...
        if (stored.expiresAtMs <= input.now || stored.record.state === "expired") {
        ...
        if (stored.record.state === "completed") {
            return stored.secret.completionBundleB64Url
                ? { kind: "completed", bundleB64Url: stored.secret.completionBundleB64Url }
                : { kind: "expired" };
        }
        ...
        return await compareAndReplaceInTx({
            ...
            record: nextRecord({
                ...
                secret: {
                    ...
                    authorizationCode: null,
                    codeVerifier: null,
                    completionBundleB64Url: input.bundleB64Url,
                },
                state: "completed",
                nextPollAtMs: input.now,
                leaseId: null,
