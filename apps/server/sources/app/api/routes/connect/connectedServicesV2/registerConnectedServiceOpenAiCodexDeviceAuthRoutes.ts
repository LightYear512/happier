...
export function registerConnectedServiceOpenAiCodexDeviceAuthRoutes(app: Fastify): void {
  app.post("/v2/connect/openai-codex/oauth/device/start", {
    ...
    return reply.send({
      ...started,
      // server-v0.2.1 called this opaque handle deviceAuthId. It intentionally aliases
      // the current server-owned transaction id, never the provider's device secret.
