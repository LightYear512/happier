...
export async function cmdSessionPreviewRegister(
  ...
  if (registerSubcommand !== 'register' || !idOrPrefix || !port || port < 1 || port > 65535) {
    throw new Error('Usage: happier session preview register <session-id-or-prefix> --port <port> [--name <name>] [--framework <id>] [--health-path <path>] [--no-rewrite-urls] [--json]');
  }
