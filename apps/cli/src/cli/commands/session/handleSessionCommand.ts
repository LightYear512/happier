...
const SESSION_HELP_BY_COMMAND = {
  ...
  'preview register': 'happier session preview register <session-id-or-prefix-or-tag> (--url <url> | --port <port>) [--name <name>] [--framework <id>] [--health-path <path>] [--no-rewrite-urls] [--json]',
  'preview android': 'happier session preview android <session-id-or-prefix-or-tag> [--device-id <adb-serial>] [--port <port>] [--poll-ms <ms>] [--device-name <name>] [--app-name <name>] [--json]',
} as const;
...
const SESSION_HELP_GROUPS: Readonly<Record<string, readonly (keyof typeof SESSION_HELP_BY_COMMAND)[]>> = {
  ...
  preview: ['preview register', 'preview android'],
};
