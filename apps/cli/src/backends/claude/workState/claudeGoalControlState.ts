/**
 * Explicit Claude goal control epoch state (plan G2).
 *
 * Replaces the previous `clearedCondition` tombstone in `claudeGoalStatus.ts`. Objective text is not
 * a valid operation identity: keying suppression on the cleared objective could not tell a stale
 * provider replay apart from a user intentionally setting the SAME objective again, so "clear -> set
 * same objective" was permanently suppressed.
 *
 ...
 * not an orderable counter), so per G2 the fallback is: suppress same-objective active replay only
 * for the current clear epoch, until a new set intent (or a different objective) occurs. The
...
export type ClaudeGoalControlIntent =
  ...
  | Readonly<{
       ...
       * same-objective active-replay suppression to the cleared goal, so a genuinely different goal
       * is not hidden. The epoch — not this text — is the operation identity.
       */
...
export function createClaudeGoalControlEpochTracker(): ClaudeGoalControlEpochTracker {
  ...
  return {
    ...
    resolveReplay(event) {
      ...
      if (event.met) {
        // A terminal (cancelled/complete) event for the cleared goal: the app already removed the
        // goal by clear intent — do not resurrect a cancelled/complete badge for it.
        return 'suppress';
      }

      // Active event during the clear window. A genuinely different objective means the provider
      // moved to a new goal — lift the clear and accept (and accept the original objective again too).
