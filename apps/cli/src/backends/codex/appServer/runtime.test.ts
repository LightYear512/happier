...
async function writeFakeCodexAppServerScript(params: Readonly<{
    ...
    const script = [
        ...
        '            const compactTaskError = { message: "Error running remote compact task: stream disconnected before completion", codexErrorInfo: "other", additionalDetails: null };',
        '            setTimeout(() => {',
        '                process.stdout.write(JSON.stringify({ method: "error", params: { threadId: msg.params?.threadId ?? null, turnId, willRetry: false, error: compactTaskError } }) + "\\n");',
        '            }, 8);',
        '            setTimeout(() => {',
        '                process.stdout.write(JSON.stringify({ method: "turn/completed", params: { threadId: msg.params?.threadId ?? null, turn: { id: turnId, status: "failed", error: compactTaskError } } }) + "\\n");',
        '            }, 8);',
        '            continue;',
        '        }',
        '        if (text === "stream-disconnected-only") {',
        '            const streamDisconnectError = { message: "stream disconnected before completion", codexErrorInfo: "other", additionalDetails: null };',
        '            setTimeout(() => {',
        '                process.stdout.write(JSON.stringify({ method: "error", params: { threadId: msg.params?.threadId ?? null, turnId, willRetry: false, error: streamDisconnectError } }) + "\\n");',
        '            }, 8);',
        '            setTimeout(() => {',
        '                process.stdout.write(JSON.stringify({ method: "turn/completed", params: { threadId: msg.params?.threadId ?? null, turn: { id: turnId, status: "failed", error: streamDisconnectError } } }) + "\\n");',
        '            }, 8);',
        '            continue;',
        '        }',
