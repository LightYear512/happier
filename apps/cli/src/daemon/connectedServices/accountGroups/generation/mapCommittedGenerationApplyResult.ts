...
export function mapCommittedGenerationApplyResult(input: Readonly<{
  ...
  if (input.result.status === 'switched' || input.result.status === 'observed_generation') {
    ...
    const hasExactTargetAdoptionProof = hasExactAcceptedConnectedServiceTargetAdoptionProof({
    ...
    const acceptedGenerationApplication = generationApplication!;
    return {
      ...
      providerAdoptedTarget: {
        ...
        proof: {
          ...
          ...(acceptedGenerationApplication.credentialRevision === undefined
            ? {}
            : { credentialRevision: acceptedGenerationApplication.credentialRevision }),
          ...(acceptedGenerationApplication.credentialFingerprint === undefined
            ? {}
            : { credentialFingerprint: acceptedGenerationApplication.credentialFingerprint }),
        },
      },
    };
  }
