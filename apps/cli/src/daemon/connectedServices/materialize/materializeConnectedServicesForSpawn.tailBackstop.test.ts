...
import {
  ...
  runSerializedMaterializationForRoot,
...
describe('materialization root-tail backstop (RR-3)', () => {
  ...
  it('bounds a joiner behind a never-settling predecessor and self-heals the queue', async () => {
    ...
    const stuck = runSerializedMaterializationForRoot(rootDir, () => new Promise<never>(() => {}));
    void stuck.catch(() => {});

    // A joiner must NOT wait forever: it fails loudly with the typed backstop error.
    const joiner = runSerializedMaterializationForRoot(rootDir, async () => 'joined');
    ...
    const fresh = runSerializedMaterializationForRoot(rootDir, async () => 'fresh');
    await vi.advanceTimersByTimeAsync(6 * 60_000 + 1_000);
  ...
  it('keeps normal serialization intact (second segment runs after the first settles)', async () => {
    ...
    const first = runSerializedMaterializationForRoot(rootDir, async () => {
    ...
    const second = runSerializedMaterializationForRoot(rootDir, async () => {
      order.push('second');
    });
