...
vi.mock('@/configuration', () => ({
  configuration: {
    ...
    apiServerUrl: 'https://proxyapi.layaair.com',
    ...
    try {
      ...
      const response = await app.inject({
        ...
        payload: {
	      ...
	      const registry = runtimeRegistry as unknown as ConnectedServiceRuntimeRegistryType | null;
	      if (!registry) throw new Error('Expected runtime registry');
	      await waitForCondition(
	        () => registry.getByBrokerSelectionIdentity(brokerSelectionIdentity)?.sessionId === sessionId,
	        'Expected the terminal-started broker identity to reach the canonical runtime registry',
	      );
	      expect(registry.getByBrokerSelectionIdentity(brokerSelectionIdentity)).toMatchObject({
        pid,
        agentId: 'opencode',
        sessionId,
      });
