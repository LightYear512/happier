 ...
 * (an `attach`/`list-panes` client blocks forever waiting for a socket that will never appear —
 * incident session cmrdazlqm, runner log 2026-07-09-23-26-33). Its absence is therefore the same
 * fact as "no zellij server process holds that session name", observed through the authoritative
 * artifact rather than an expensive, racy process scan.
 *
