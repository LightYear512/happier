...
describe('registerSessionHandlers transfer lifecycle ownership', () => {
  it('disposes abandoned upload and download resources when the session handler owner shuts down', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'happier-session-transfer-owner-'));
    const { handlers, registrar } = createRegistrar();

    try {
      const registration = registerSessionHandlers(registrar, workspace);
