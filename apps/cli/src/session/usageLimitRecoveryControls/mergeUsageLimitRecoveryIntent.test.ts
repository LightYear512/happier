...
describe('mergeUsageLimitRecoveryIntent', () => {
  ...
  ])('retains the stricter same-attempt execution cap for $left and $right', ({ left, right, expected }) => {
    const first = intent({ attemptCount: 3, maxAttempts: left, status: 'exhausted' });
    const second = intent({ attemptCount: 3, maxAttempts: right, status: 'waiting' });

    expect(mergeUsageLimitRecoveryIntent(first, second)?.maxAttempts).toBe(expected);
    expect(mergeUsageLimitRecoveryIntent(second, first))
      .toEqual(mergeUsageLimitRecoveryIntent(first, second));
  });
