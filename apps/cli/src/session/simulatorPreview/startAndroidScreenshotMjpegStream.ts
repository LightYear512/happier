...
import { normalizeAndroidScreenshotFrame, type AndroidScreenshotFrame } from './androidScreenshotFrame';
import { createAndroidScreencapFrameCapture } from './createAndroidScreencapFrameCapture';

export type AndroidScreenshotMjpegStream = Readonly<{
  host: string;
  port: number;
  frameUrl: string;
  streamUrl: string;
...
function writeMultipartFrame(response: ServerResponse, boundary: string, frame: AndroidScreenshotFrame): void {
...
export async function startAndroidScreenshotMjpegStream(
  ...
  let nextClientId = 1;
  let latestFrame: AndroidScreenshotFrame | null = null;
  let latestError: Error | null = null;
  let closed = false;
  let captureInFlight = false;

  const broadcastFrame = (frame: AndroidScreenshotFrame) => {
    for (const client of clients.values()) {
      writeMultipartFrame(client.response, boundary, frame);
    }
  };
  ...
  const poll = async () => {
    ...
    try {
      const frame = normalizeAndroidScreenshotFrame(await captureFrame());
      latestFrame = frame;
      latestError = null;
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/frame.jpg') {
      ...
      if (!latestFrame) {
        ...
        res.end(latestError?.message ?? 'No Android screenshot frame is available yet');
        return;
      }
      res.statusCode = 200;
    ...
    if (req.method === 'GET' && url.pathname === '/stream.mjpeg') {
      ...
      if (latestFrame) {
        writeMultipartFrame(res, boundary, latestFrame);
      } else {
        void poll();
      }
      req.once('close', () => {
