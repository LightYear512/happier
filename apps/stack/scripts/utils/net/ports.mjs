...
export async function listListenPidsWithStatus(
  ...
  if (platform === 'win32') {
    if (hasProcessGroupFilter) {
      return listenerObservation(
        'unsupported',
        [],
        'process-group-listener-discovery-unsupported',
      );
    }
    let resolved;
    try {
      resolved = await resolveCommandPathImpl('netstat', { timeoutMs, env });
    } catch (error) {
    ...
    try {
      const raw = await runCaptureImpl(resolved, ['-ano', '-p', 'TCP'], { timeoutMs, signal, env });
      const observedPids = parseWindowsNetstatListenPids(raw, port);
      return listenerObservation(
        'ok',
  ...
  for (const candidate of candidates) {
    ...
    let resolved;
    try {
      resolved = await resolveCommandPathImpl(candidate, { timeoutMs, env });
    } catch (error) {
      return classifyListenerDiscoveryError(error);
    }
...
function listLocalInterfaceBindHosts(host, networkInterfacesImpl, platform) {
  ...
  // A raw address object cannot prove that omitted interfaces were absent rather than
  // unobserved. Only the canonical OS boundary (or an explicitly complete boundary adapter)
  // may authorize cross-interface bind evidence.
  ...
  // A wildcard bind does not prove the absence of an interface-specific listener on macOS.
  // Without at least one usable OS interface entry, cross-interface availability is unknown.
  if (!hasIpv4 && !hasIpv6) return null;
  const requestedHostFamily = net.isIP(String(host ?? '').split('%')[0]);
  if (requestedHostFamily === 4) hasIpv4 = true;
  if (requestedHostFamily === 6) hasIpv6 = true;
  if (hasIpv4) addHost('0.0.0.0');
  if (hasIpv6) addHost('::');
  return hosts;
}

async function probeTcpPortBindingWithinDeadline({
  port,
  host,
  remainingMs,
...
export async function pickNextFreeTcpPort(
  ...
  const throwDeadlineExceeded = () => {
    const error = new Error(
      `[local] unable to find a free TCP port within ${Math.max(1, Number(totalTimeoutMs) || 1)}ms`,
    );
  ...
  for (let i = 0; i < tries; i++) {
    if (remainingMs() <= 0) throwDeadlineExceeded();
    if (reservedPorts.has(port)) {
      port += 1;
      continue;
    }

    // Fresh allocation and post-stop reuse share one complete-interface availability owner.
    // eslint-disable-next-line no-await-in-loop
    const availability = await observeTcpPortAvailabilityImpl(port, {
      host,
      deadline,
