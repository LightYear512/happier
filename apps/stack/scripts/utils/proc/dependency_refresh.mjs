...
export async function inspectDependencyRefresh({ installDir, componentDir = installDir }) {
  ...
  const hasCanonicalMarker = markerState?.version === 2 && Array.isArray(markerState.inputs);
  if (hasCanonicalMarker) {
    return {
      required: !snapshotsMatch(markerState.inputs, inputSnapshot),
      inputPaths,
      inputSnapshot,
      markerPath,
    };
  ...
  // Legacy timestamp boundaries cannot prove additions or deletions. Refresh once to establish
  // the canonical full snapshot, even when .yarn-integrity appears newer than every current input.
