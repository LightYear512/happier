...
function buildMemoryAttributionLaunchEnv(env) {
  return {
    ...env,
    SIMCTL_CHILD_MallocStackLogging: '1',
    SIMCTL_CHILD_MallocStackLoggingNoCompact: '1',
  };
}
...
export async function runIosProfilingRuntime({
  ...
  const launchEnv = memoryAttribution ? buildMemoryAttributionLaunchEnv(env) : env;
  const launchArgs = [
    'simctl',
    'launch',
    '--terminate-running-process',
    ...(memoryAttribution ? ['--checked-allocations'] : []),
    simulator.udid,
    bundleId,
  ];
  await run('xcrun', launchArgs, { cwd: projectDir, env: launchEnv });
  ...
  return {
    ...
    debugDylib,
    device: simulator,
    memoryAttribution: memoryAttribution === true,
  };
}
