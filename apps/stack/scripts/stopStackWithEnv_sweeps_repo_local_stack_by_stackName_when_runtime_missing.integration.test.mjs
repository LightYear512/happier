...
import { terminateProcessGroup, terminateProcessPid } from './utils/proc/terminate.mjs';
...
test('stopStackWithEnv repo-local fallback sweeps legacy infra without stopping session processes', async (t) => {
  ...
  const legacyInfraFingerprint = readProcessInstanceFingerprintSync(legacyInfra.pid);
  assert.ok(legacyInfraFingerprint, 'expected legacy infra process incarnation');
  ...
  const incarnationMismatchInfra = spawnOwnedSleep({
    env: {
      ...process.env,
      HAPPIER_STACK_STACK: stackName,
      HAPPIER_STACK_REPO_DIR: repoDir,
  ...
  children.push(incarnationMismatchInfra);
  await waitForProcessAlive({
    pid: incarnationMismatchInfra.pid,
    timeoutMs: 2_000,
    intervalMs: 25,
    label: 'repo-local incarnation-mismatch process (pre-stop)',
  });
  ...
  const incarnationProbeCounts = new Map();
  const readFingerprintWithReplacement = (pid) => {
    const fingerprint = readProcessInstanceFingerprintSync(pid);
    if (pid !== incarnationMismatchInfra.pid || !fingerprint) return fingerprint;
    const count = (incarnationProbeCounts.get(pid) ?? 0) + 1;
    incarnationProbeCounts.set(pid, count);
    return count === 1 ? fingerprint : `${fingerprint}:successor`;
  };
  const actions = await stopStackWithEnv(stopInput, {
    observePsEnvLineImpl: observeMixedIdentity,
    readProcessInstanceFingerprintImpl: readFingerprintWithReplacement,
  ...
  assert.ok(
    isAlive(incarnationMismatchInfra.pid),
    `post-proof process-incarnation replacement must not authorize repo-local direct sweep of pid ${incarnationMismatchInfra.pid}`,
  );
  assert.deepEqual(
    actions.sweep.skipped.find((entry) => entry.pid === incarnationMismatchInfra.pid),
    { pid: incarnationMismatchInfra.pid, reason: 'process_instance_mismatch' },
  );
  const legacyTermination = terminationCalls.find((call) => call.options?.identityPid === legacyInfra.pid);
  assert.equal(legacyTermination?.options?.processInstanceFingerprint, legacyInfraFingerprint);
  assert.ok(
