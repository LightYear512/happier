...
test('startup reclaims an abandoned cleanup executor and finalizes before ordinary start claim', async (t) => {
  ...
  const ownerStartedAt = '2026-07-17T08:00:00.000Z';
  await writeRuntimeState(fixture.runtimeStatePath, {
    version: 1, stackName: fixture.stackName, ownerPid, startedAt: ownerStartedAt,
    processes: {},
  ...
  await writeFile(join(fixture.baseDir, 'stack.cleanup-executor.lock'), JSON.stringify({
    pid: 999_998, createdAtMs: Date.now(), updatedAtMs: Date.now(),
  }), 'utf8');
...
test('ordinary startup without a stop marker does not acquire the cleanup executor lock', async (t) => {
  ...
  await writeRuntimeState(fixture.runtimeStatePath, {
    version: 1, stackName: fixture.stackName, ownerPid: null, processes: {}, stopRequest: null,
  });
  await writeFile(join(fixture.baseDir, 'stack.cleanup-executor.lock'), JSON.stringify({
    pid: process.pid, createdAtMs: Date.now(), updatedAtMs: Date.now(),
  }), 'utf8');
...
test('watchdog stale preserveDaemon false cannot downgrade a fresher true stop request', async (t) => {
  ...
  assert.deepEqual(result.stopAttribution, {
    requestedBy: 'fresh restart',
    reason: 'preserve daemon for successor',
  });
...
test('stopStackWithEnv requests the lifecycle owner before fallback child termination', async (t) => {
  ...
  const owner = await fixture.spawnScript(`
    const { writeFileSync } = require('node:fs');
    process.on('SIGTERM', () => { writeFileSync(${JSON.stringify(ownerMarker)}, 'owner-first'); process.exit(0); });
    writeFileSync(${JSON.stringify(ownerReady)}, 'ready');
    setInterval(() => {}, 1000);
  `, 'lifecycle-owner');
  const child = await fixture.spawnScript(`
    const { existsSync, writeFileSync } = require('node:fs');
    process.on('SIGTERM', () => { writeFileSync(${JSON.stringify(childObservation)}, existsSync(${JSON.stringify(ownerMarker)}) ? 'owner-first' : 'child-first'); process.exit(0); });
...
test('stopStackWithEnv stops the exact named-stack daemon lifecycle scope', async (t) => {
  const stackName = 'named-stop-scope';
  const cliIdentity = 'account-b';
  const fixture = await createStopFixture(t, { stackName });
  const markerPath = join(fixture.baseDir, 'daemon-stop-env.json');
  await writeStubHappierCliFiles(fixture.repoRoot, {
    packageJsonContent: '{}\n',
    distIndexScript: `
  ...
  try {
    ...
    const result = await stopStackWithEnv({
      ...
      env: {
        ...
        HAPPIER_STACK_CLI_HOME_DIR: fixture.cliHomeDir,
        HAPPIER_STACK_REPO_DIR: fixture.repoRoot,
        DAEMON_STOP_ENV_MARKER: markerPath,
      },
      json: true,
      noDocker: true,
    });
    ...
    assert.deepEqual(JSON.parse(await readFile(markerPath, 'utf8')), {
      lifecycleScopeId: buildStackStableScopeId({ stackName, cliIdentity }),
      stackName,
      cliIdentity,
    });
  } finally {
