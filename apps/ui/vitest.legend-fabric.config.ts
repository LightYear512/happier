 ...
 * new architecture.
 *
 * `vitest.legend-native.config.ts` is left on old architecture on purpose. Its 1600+ existing
 * assertions were written against `PositionViewAnimated` rows and the `.measure()` layout fallback;
 * flipping it would silently re-interpret all of them. That lane keeps old-architecture native
 * coverage; this one adds the configuration the user actually runs.
 *
