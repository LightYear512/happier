...
export function hasCompletedLegacySessionOrganizationImport(scope: AccountSettingsScope): boolean {
    return getPersistenceStorage().getString(legacySessionOrganizationImportMarkerKey(scope)) === '1';
}

export function markLegacySessionOrganizationImportComplete(scope: AccountSettingsScope): void {
...
function pruneImportedCategories(params: {
    ...
    const existingTagIdsAndKeys = new Set<string>();
    for (const tag of snapshot.tags) {
        if (tag.archivedAt != null) continue;
        existingTagIdsAndKeys.add(tag.tagId);
        existingTagIdsAndKeys.add(tag.tagKey);
    }
    ...
    return {
        request: {
            ...
            tags: params.request.tags.filter((tag) => {
                const tagId = tag.tagId ?? tag.tagKey;
                return !existingTagIdsAndKeys.has(tagId) && !existingTagIdsAndKeys.has(tag.tagKey);
            }),
