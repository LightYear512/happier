 ...
 * server. Measured on the user's live database (2026-08-07, read-only): 7 such rows, 3 of them
 * `status: 'queued'` / `deliveryState: 'blocked'` /
...
describe('pendingQueueV2 durable pending/committed coexistence', () => {
    ...
    /**
     * SESSION OPEN. `sessionMessages` has no warm cache (`sync/domains/state/warmCachePersistence.ts`
     * persists session metadata only), so the transcript store is EMPTY when `SessionView` fires the
     * refresh, and the first page lands inside the GET's window (measured RTT for that GET in the
     * C6 device traces: 155–264 ms). The twin the page carries is old news, not a settlement.
     */
     ...
     *
     * `isLoaded === true` with NOTHING loaded is reachable in the current bytes: `sync/sync.ts`'s
     * `fetchMessages` calls `applyMessagesLoaded(sessionId)` and returns without applying a single
     * message when the session cannot be resolved to a known owner server, and
     * `sync/engine/sessions/syncSessions.ts`'s `markMessagesLoaded` runs after the initial page
     * pipeline even when that page was empty. In that state the only mark left is `session.seq`,
     * which `sync/domains/state/warmCacheAdapters.ts` persists (and rehydrates) while it caches
     * NEITHER `sessionMessages` nor `sessionPending` — so it can be arbitrarily stale-low, and a
     * durable twin then sits ABOVE it and the live blocked row is hidden.
     *
