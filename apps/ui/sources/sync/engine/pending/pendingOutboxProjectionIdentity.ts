...
export function isPendingOutboxProjectionForIdentity(
    message: Pick<PendingMessage, 'id' | 'localId' | 'pendingOutboxScope' | 'pendingOutboxQuarantineReason'>,
    identity: PendingOutboxProjectionIdentity,
): boolean {
    const messageLocalId = message.localId ?? message.id;
    return message.pendingOutboxQuarantineReason === undefined
        && messageLocalId === identity.localId
        && isPendingOutboxProjectionInScope(message, identity.outboxScope);
}

export function isPendingOutboxQuarantineProjectionForIdentity(
    message: Pick<PendingMessage, 'id' | 'localId' | 'pendingOutboxScope' | 'pendingOutboxQuarantineReason'>,
    identity: PendingOutboxProjectionIdentity,
): boolean {
    const messageLocalId = message.localId ?? message.id;
