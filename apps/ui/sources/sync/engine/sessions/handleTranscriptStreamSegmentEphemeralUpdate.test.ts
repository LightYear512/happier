...
describe('handleTranscriptStreamSegmentEphemeralUpdate delta reconstruction', () => {
    ...
    it('reconstructs accumulated text from a snapshot plus chained deltas through the same apply path', async () => {
        ...
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainSnapshotUpdate(sessionId, 'Hello', { tick: 1 }),
        });
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, ' wor', { tick: 2, baseLength: 5 }),
        });
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, 'ld', { tick: 3, baseLength: 9 }),
        });
    ...
    it('drops deltas for unknown segments until a snapshot establishes assembly state', async () => {
        ...
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, 'orphan', { tick: 2, baseLength: 5 }),
        });
        expect(applyMessages).not.toHaveBeenCalled();

        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainSnapshotUpdate(sessionId, 'Hello orphan', { tick: 3 }),
        });
        expect(applyMessages).toHaveBeenCalledTimes(1);
    ...
    it('drops deltas on tick gaps and stays desynced until the next snapshot resyncs', async () => {
        ...
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, '!', { tick: 4, baseLength: 5 }),
        });
        expect(applyMessages).toHaveBeenCalledTimes(1);

        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainSnapshotUpdate(sessionId, 'Hello world!', { tick: 5 }),
        });
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, '?', { tick: 6, baseLength: 12 }),
        });
    ...
    it('drops deltas whose base length does not match the assembled text', async () => {
        ...
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, ' world', { tick: 2, baseLength: 4 }),
        });
    ...
    it('evicts assembly state when a snapshot reports a completed segment', async () => {
        ...
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainSnapshotUpdate(sessionId, 'Hello', { tick: 1, state: 'complete' }),
        });
        await handleTranscriptStreamSegmentEphemeralUpdate({
            ...params,
            update: buildPlainDeltaUpdate(sessionId, ' late', { tick: 2, baseLength: 5 }),
        });
