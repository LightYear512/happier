...
describe('sync tail-reset pagination hole (discontinuity walk)', () => {
    ...
    it('keeps the deepest stale-prefix boundary across terminal exhaustion and a later tail reset', async () => {
        ...
        // A later reset opens a new island above the previously visible island. Its first
        // older page bridges only to that newer island, not to the deepest stale prefix
        // at seq 410. The display floor must therefore remain closed above the old prefix.
