...
const ORGANIZATION_KEY = 'session-organization-warm-cache-v1:server-a:account-a';
const MACHINE_KEY = 'machine-display-warm-cache-v1:server-a:account-a';
const LEGACY_PINNED_KEY = 'session-list-pinned-ids-warm-cache-v1:server-a:account-a';
...
const ORGANIZATION_SNAPSHOT: SessionOrganizationSnapshot = {
    schemaVersion: 1,
    version: 3,
    pins: [],
    folders: [],
...
describe('warmCachePersistence at-rest encryption', () => {
    ...
    it('writes decrypted session content only through an encrypted MMKV instance', async () => {
        ...
        persistence.saveSessionOrganizationWarmCacheSnapshot('server-a', 'account-a', ORGANIZATION_SNAPSHOT);
    ...
    it('deletes warm-cache keys previous builds wrote in plaintext, and nothing else', async () => {
        ...
        legacy.set(ORGANIZATION_KEY, JSON.stringify({ version: 0, pinnedSessionIds: ['s1'] }));
        legacy.set(MACHINE_KEY, JSON.stringify({ m1: { host: 'leeroy-mbp' } }));
        legacy.set(LEGACY_PINNED_KEY, JSON.stringify(['s1']));
    ...
    it('degrades to a cold boot instead of throwing when the key is unavailable', async () => {
        ...
        expect(persistence.loadSessionOrganizationWarmCacheSnapshot('server-a', 'account-a')).toBeNull();
...
describe('warmCachePersistence on web', () => {
    ...
    it('restores the machine displays and the organization snapshot too, not only the session list', async () => {
        stubWebRuntime();
        try {
            const persistence = await importWarmCachePersistence();
            await persistence.prepareWarmCacheStorage();
            ...
            expect(reloaded.loadSessionOrganizationWarmCacheSnapshot('server-a', 'account-a')).toEqual(ORGANIZATION_SNAPSHOT);
        } finally {
            vi.unstubAllGlobals();
        }
    });
