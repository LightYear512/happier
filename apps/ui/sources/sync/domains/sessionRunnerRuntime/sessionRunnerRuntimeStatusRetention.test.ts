...
describe('sessionRunnerRuntimeStatusRetention', () => {
    ...
    it('clears retained snapshots and invalidates pending refresh tickets', () => {
        const retention = createSessionRunnerRuntimeStatusRetention();
        const retainedIdentity = buildIdentity();
        const pendingIdentity = buildIdentity({ sessionId: 'session-pending' });
        retention.completeRefresh(
            retention.beginRefresh(retainedIdentity),
            buildRuntimeState(retainedIdentity),
        );
        const pendingRefresh = retention.beginRefresh(pendingIdentity);

        retention.clear();
        retention.completeRefresh(
            pendingRefresh,
            buildRuntimeState(pendingIdentity),
        );

        expect(retention.read(retainedIdentity)).toBeNull();
        expect(retention.read(pendingIdentity)).toBeNull();
    });
});
