...
describe("createPublicShareWithClientToken", () => {
  ...
  it("fails closed and clears the attempted token when create fails before an existing share changes", async () => {
  ...
  it("does not attach an attempted token to a concurrently returned prior share", async () => {
    let cachedToken: string | null = null;
    const tokenCache = { get: () => cachedToken, set: (token: string | null) => (cachedToken = token) };
    const apiWithPriorShareSentinel = {
      createPublicShare: async () => {
        throw new Error("conflict");
      },
      getPublicShare: async () => createShare({ id: "share-prior", token: null }),
    };
    ...
    await expect(createPublicShareWithClientToken({
      ...
      isConsentRequired: true,
      tokenCache,
      generateTokenHex: () => "tok_concurrent_attempt",
      api: apiWithPriorShareSentinel,
