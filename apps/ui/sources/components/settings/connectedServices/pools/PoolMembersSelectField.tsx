...
export const PoolMembersSelectField = React.memo(function PoolMembersSelectField(
    ...
    const committedSelection = React.useMemo(() => new Set(selectedProfileIds), [selectedProfileIds]);
    const selected = draft ?? committedSelection;

    const selectedCount = React.useMemo(
        () => candidates.reduce((count, candidate) => (selected.has(candidate.profileId) ? count + 1 : count), 0),
        [candidates, selected],
    );
    ...
    const commitDraft = React.useCallback(async (nextSelected: ReadonlySet<string>) => {
        ...
        const nextSelectedProfileIds = candidates
            .map((candidate) => candidate.profileId)
            .filter((profileId) => nextSelected.has(profileId));
    ...
    const handleOpenChange = React.useCallback((next: boolean) => {
        ...
        if (next) {
            ...
            setDraft(new Set(selectedProfileIds));
            return;
        }
        ...
        setDraft(null);
    }, [commitDraft, draft, selectedProfileIds]);

    const handleSelect = React.useCallback((profileId: string) => {
        setDraft((current) => {
            const next = new Set(current ?? selectedProfileIds);
    ...
    const items = React.useMemo(() => candidates.map((candidate) => {
        const checked = selected.has(candidate.profileId);
        return {
            id: candidate.profileId,
            testID: `connected-services-pool-detail:members:option:${candidate.profileId}`,
