...
describe('MeterBar', () => {
    it.each([
        ...
        // The fill must read the token directly — no rgba/opacity
        // transform of a theme token (web var-ification trap).
        expect(String(style.backgroundColor)).not.toMatch(/rgba/i);
    });

    it('maps the fill fraction (consumed/progress) to the fill width percentage', async () => {
        const { MeterBar } = await import('./MeterBar');
    ...
    it('clamps fillFraction into the 0..1 range', async () => {
        const { MeterBar } = await import('./MeterBar');
