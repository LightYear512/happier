...
describe('partitionItemsIntoColumns', () => {
    it('distributes round-robin so a ranked list still reads left-to-right', () => {
        // Attention-first ordering must survive: the two worst rows stay on the
        // first visual line, not stacked at the top of one column.
        expect(partitionItemsIntoColumns(['a', 'b', 'c', 'd', 'e'], 2)).toEqual([
            ['a', 'c', 'e'],
            ['b', 'd'],
        ]);
    });

    it('returns one bucket holding everything for a single column', () => {
        expect(partitionItemsIntoColumns(['a', 'b', 'c'], 1)).toEqual([['a', 'b', 'c']]);
    });

    it('keeps trailing columns present but empty when items run out', () => {
        expect(partitionItemsIntoColumns(['a'], 3)).toEqual([['a'], [], []]);
    });

    it('handles an empty list without inventing buckets', () => {
        expect(partitionItemsIntoColumns([], 2)).toEqual([[], []]);
    });
});
