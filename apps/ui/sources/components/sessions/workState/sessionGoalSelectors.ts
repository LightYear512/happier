import { resolveActiveSessionGoalItem } from './sessionWorkStatePresentation';
 ...
 * `findGoal()` that could diverge from `resolveActiveSessionGoalItem`.
 */
...
/**
 * The session's CURRENT goal (active-only) for "current goal" surfaces such as the chip label.
 * Delegates to the canonical `resolveActiveSessionGoalItem` so the controller and the presentation
 * layer agree on what "current" means (no divergent predicate).
 */
export function resolveCurrentGoalItem(snapshot: SessionWorkStateSnapshot | null): SessionWorkStateItem | null {
    return resolveActiveSessionGoalItem(snapshot);
}
