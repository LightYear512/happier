...
describe('resolveTranscriptTargetWindowDisplay', () => {
    ...
    it('keeps a gap larger than the legacy scan cap contiguous when the window owner covers the range', () => {
        ...
        const isSeqRangeLoaded = (fromInclusive: number, toInclusive: number) => (
        ...
        expect(isSeqRangeLoaded(101, 899)).toBe(true);
    });

    it('still splits a large gap when the window owner does not cover that range', () => {
        const items = [
            { id: 'msg-100', seq: 100 },
            { id: 'msg-900', seq: 900 },
        ] as const;
        ...
        expect(result.omittedAfterCount).toBe(1);
