 ...
 *
 * Defect this pins: the sampling interval was installed only when
 * `usesNativeFlashListBottomMaintenance` was true. That flag means
 * `rendererKind === 'flashList' && platformOS !== 'web'`, and the shipped renderer is Legend
 * (`transcriptLegendListSpikeSurface: 'off'`), so the interval never installed on any real native
 * open. `nativeMountSettleStable` stayed permanently false and `nativeMountSettleDeadlineReached`
 * was never set — the settle fact consumed by the reveal gate was dead.
 *
 * Mount settle is a native placement fact, not a renderer fact: its inputs (list layout commits and
 * composer inset changes) are produced identically under Legend and FlashList. The producer is
 * therefore gated on native only.
 *
...
function createHarness(overrides?: Readonly<{ platformOS?: string }>) {
    ...
    const closeEntryViewportOwnership = vi.fn();
    const flushPendingNativeMountSettleBottomPin = vi.fn();
    const scheduleNativePaintReleaseForEntryRestore = vi.fn();
    const nativeMountSettleDeadlineReachedRef = { current: false };
    const nativeMountSettleAutoPinSuppressedRef = { current: false };
    const pendingNativeMountSettleBottomPinHostRef = { current: { current: false } };
...
describe('useTranscriptNativeMountSettleLifecycle', () => {
    ...
    it('samples mount settle on native under the renderer-owned (Legend) positioning surface', async () => {
    ...
    it('reaches the mount-settle deadline exactly once when stability never arrives', async () => {
        ...
        expect(harness.closeEntryViewportOwnership).toHaveBeenCalledWith('deadline');
