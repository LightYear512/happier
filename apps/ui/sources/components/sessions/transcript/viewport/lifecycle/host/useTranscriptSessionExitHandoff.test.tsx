...
type HarnessProps = Readonly<{
    captureAtExit: (snapshot: ReturnType<typeof readExitSnapshot>) => void;
    committedProjectionRefs: CommittedProjectionRefs;
    currentSessionIdRef: { current: string };
    geometryHeight: number;
    geometryHeightRef: React.MutableRefObject<number>;
    observeChildLayoutProjection: (projection: ProjectionSnapshot) => void;
...
type ProjectionSnapshot = Readonly<{
    canonicalWindowedItems: readonly string[];
    displayItems: readonly string[];
    entrySliceWithheldCount: number;
    listData: readonly string[];
    renderWindowIndexMap: Readonly<{ sessionId: string }>;
}>;

type CommittedProjectionRefs = Readonly<{
    canonicalWindowedItemsRef: React.MutableRefObject<readonly string[]>;
    itemsRef: React.MutableRefObject<readonly string[]>;
    entrySliceWithheldCountRef: React.MutableRefObject<number>;
    listDataRef: React.MutableRefObject<readonly string[]>;
    renderWindowIndexMapRef: React.MutableRefObject<Readonly<{ sessionId: string }>>;
}>;
...
function readProjectionSnapshot(refs: CommittedProjectionRefs): ProjectionSnapshot {
    return {
        canonicalWindowedItems: refs.canonicalWindowedItemsRef.current,
        displayItems: refs.itemsRef.current,
        entrySliceWithheldCount: refs.entrySliceWithheldCountRef.current,
        listData: refs.listDataRef.current,
        renderWindowIndexMap: refs.renderWindowIndexMapRef.current,
    };
}

function readExitSnapshot(
    refs: CommittedProjectionRefs,
    geometryHeightRef: React.MutableRefObject<number>,
) {
    return {
        geometryHeight: geometryHeightRef.current,
        projection: readProjectionSnapshot(refs),
    };
}

function PhysicalListBoundary(props: Readonly<{
    geometryHeight: number;
    geometryHeightRef: React.MutableRefObject<number>;
}>) {
    React.useLayoutEffect(() => {
        props.geometryHeightRef.current = props.geometryHeight;
    });
    return null;
}

function ChildLayoutProbe(props: Readonly<{
    committedProjectionRefs: CommittedProjectionRefs;
    observe: (projection: ProjectionSnapshot) => void;
}>) {
    React.useLayoutEffect(() => {
        props.observe(readProjectionSnapshot(props.committedProjectionRefs));
    });
    return null;
}
...
function SessionHandoffHarness(props: HarnessProps) {
    ...
    const exitCurrentSession = React.useCallback(() => {
        captureAtExitRef.current(readExitSnapshot(
            props.committedProjectionRefs,
            props.geometryHeightRef,
        ));
    }, [props.committedProjectionRefs, props.geometryHeightRef]);
    ...
    return (
        <>
            <ChildLayoutProbe
                committedProjectionRefs={props.committedProjectionRefs}
                observe={props.observeChildLayoutProjection}
            />
            <PhysicalListBoundary
                key={props.sessionId}
                geometryHeight={props.geometryHeight}
                geometryHeightRef={props.geometryHeightRef}
            />
        </>
    );
}
...
describe('useTranscriptSessionExitHandoff', () => {
    ...
    it('keeps a committed projection authoritative through an abandoned same-session render, then atomically publishes the next commit', async () => {
        const captureA = vi.fn();
        const captureB = vi.fn();
        const observeChildLayoutProjection = vi.fn();
        const projectionA: ProjectionSnapshot = {
            canonicalWindowedItems: ['canonical-a'],
            displayItems: ['display-a'],
            entrySliceWithheldCount: 1,
            listData: ['item-a'],
            renderWindowIndexMap: { sessionId: 'session-a' },
        };
        const projectionB: ProjectionSnapshot = {
            canonicalWindowedItems: ['canonical-b'],
            displayItems: ['display-b'],
            entrySliceWithheldCount: 2,
            listData: ['item-b'],
            renderWindowIndexMap: { sessionId: 'session-b' },
        };
        const committedProjectionRefs: CommittedProjectionRefs = {
            canonicalWindowedItemsRef: { current: projectionA.canonicalWindowedItems },
            itemsRef: { current: projectionA.displayItems },
            entrySliceWithheldCountRef: { current: projectionA.entrySliceWithheldCount },
            listDataRef: { current: projectionA.listData },
            renderWindowIndexMapRef: { current: projectionA.renderWindowIndexMap },
        };
        const currentSessionIdRef = { current: 'session-a' };
        const geometryHeightRef = { current: 100 };
        let tree!: renderer.ReactTestRenderer;
        ...
        expect(readProjectionSnapshot(committedProjectionRefs)).toEqual(projectionA);
        expect(geometryHeightRef.current).toBe(100);
        ...
        expect(captureB).not.toHaveBeenCalled();
        expect(currentSessionIdRef.current).toBe('session-a');
        expect(readProjectionSnapshot(committedProjectionRefs)).toEqual(projectionB);
        expect(observeChildLayoutProjection).toHaveBeenLastCalledWith(projectionB);

        await act(async () => {
            tree.unmount();
        });
        expect(captureA).not.toHaveBeenCalled();
        expect(captureB).toHaveBeenCalledExactlyOnceWith({
            geometryHeight: 200,
            projection: projectionB,
        });
    });
