 ...
 * the reader takes the viewport back. Genuine user movement (the scroll-ingress
 * classification that already separates a real gesture from the app's own
 * programmatic writes) releases it immediately, so normal scrollspy resumes on
 * the very same frame; a session change or the anchor leaving the anchor set
 * releases it too, because neither can still describe where the reader is.
 */
