 ...
 *
 * `signature-change` mutations are content/streaming re-signatures, not user toggles — they
 * never arm (the tail-follow and MVCP own those).
 */
export function resolveRowLayoutMutationViewportOwnershipAction(params: Readonly<{
    ...
    if (params.localHeightChangeRestoreOwner !== 'renderer') return 'none';
    if (params.reason !== 'expand' && params.reason !== 'collapse') return 'none';
