...
const items: readonly Item[] = [
    ...
    { id: 'header-third', owns: [], contains: ['hidden-tool'] },
];
...
describe('sidechain jump to message', () => {
    ...
    it('centers the loaded target and never scrolls into footer chrome', () => {
    ...
    it('falls back to the estimated animated offset when scrollToIndex rejects', async () => {
        ...
        expect(scrollToOffset).toHaveBeenCalledWith({ offset: 264, animated: true });
        expect(scrollToEnd).not.toHaveBeenCalled();
    });
