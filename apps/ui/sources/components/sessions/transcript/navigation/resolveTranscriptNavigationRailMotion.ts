...
// change produces an identical motion value for every marker outside the
// window and the memoized markers skip re-rendering — the bounded update
// window is a property of the resolver, not of an imperative writer.
...
/**
 * The rail's easing lives in CSS on the marker's own stable class, so React
 * stays the single owner of every motion channel: it renders the resolved
 * target and the browser interpolates. Reduced motion collapses the duration
 * to zero rather than removing the declaration, so the same style path serves
 * both preferences. Native never renders the rail
 * (`deriveTranscriptNavigationRailLayout` hides it off web), so there is no
 * setNativeProps counterpart to keep in sync.
 */
