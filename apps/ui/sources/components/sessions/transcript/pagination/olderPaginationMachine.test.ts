...
describe('resolveItemsToNewerEdge', () => {
    it('returns the remaining item count after a genuine visible subset', () => {
        expect(resolveItemsToNewerEdge({ firstSourceIndex: 40, lastSourceIndex: 48 }, 200)).toBe(151);
        expect(resolveItemsToNewerEdge({ firstSourceIndex: 191, lastSourceIndex: 199 }, 200)).toBe(0);
    });

    it('shares the older-edge invalid and whole-range rejection contract', () => {
        expect(resolveItemsToNewerEdge({ firstSourceIndex: 0, lastSourceIndex: 199 }, 200)).toBeNull();
        expect(resolveItemsToNewerEdge({ firstSourceIndex: 0, lastSourceIndex: 200 }, 200)).toBeNull();
        expect(resolveItemsToNewerEdge(null, 200)).toBeNull();
    });
});
