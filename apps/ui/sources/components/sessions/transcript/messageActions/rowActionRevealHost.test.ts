        ...
        matchMedia: (query: string) => {
            ...
            return {
                ...
                removeEventListener: (event: string, listener: MediaChangeListener) => {
                    ...
                    const index = pointerState.changeListeners.indexOf(listener);
                    if (index >= 0) pointerState.changeListeners.splice(index, 1);
                },
            };
        },
    };
...
describe('readCoarsePrimaryPointer', () => {
    ...
    it('caches per runtime but re-resolves after the primary pointer changes', async () => {
        ...
        try {
            ...
            expect(readCoarsePrimaryPointer()).toBe(false);
            // Cached: a second read must not re-evaluate the media queries.
