...
async function importScopedAuthKeys() {
    const { TokenStorage } = await import('./tokenStorage');
    // Probe the scope layout through the public surface: a write lands on the primary scope, and the
    // legacy scopes are whatever else `getCredentials` is willing to migrate from.
    return TokenStorage;
}
...
describe('TokenStorage credential lookup (native keychain)', () => {
    ...
    it('probes every legacy scope concurrently instead of one keychain round trip at a time', async () => {
        ...
        const TokenStorage = await importScopedAuthKeys();
