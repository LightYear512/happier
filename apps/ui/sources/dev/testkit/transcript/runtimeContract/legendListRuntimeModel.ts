...
export class LegendListRuntimeModel {
    ...
    computeVisibleIndices(): { startIndex: number; endIndex: number } {
        let startIndex = 0;
        let endIndex = Math.max(0, this.rows.length - 1);
        let foundStart = false;
        for (let index = 0; index < this.rows.length; index += 1) {
            const top = this.topOfIndex(index);
            const bottom = top + (this.rows[index]?.heightPx ?? 0);
            if (!foundStart && bottom > this.rawOffsetY + EPSILON_PX) {
                startIndex = index;
                foundStart = true;
            }
            if (top <= this.rawOffsetY + this.layoutHeightPx + EPSILON_PX) {
                endIndex = index;
            }
        }
    ...
    scrollToOffset(params: Readonly<{ offset: number }>): Readonly<{ appliedOffsetY: number }> {
        this.rawOffsetY = clampOffset(params.offset, this.maxScrollableExtent());
        this.captureFirstVisibleAnchor();
        return { appliedOffsetY: this.rawOffsetY };
    }
