...
describe('reduceUiDeploymentFreshness', () => {
    it('warns only after observing two different valid deployment identifiers', () => {
        ...
        expect(reduceUiDeploymentFreshness(baseline, 'version=1')).toEqual(baseline);
        expect(reduceUiDeploymentFreshness(baseline, 'deployment_B_123456')).toEqual({ baselineId: 'deployment_A_123456', updateAvailable: true });
    });
});
