...
describe('buildAppUpdateStatusModel', () => {
    it('offers a refresh when the same UI origin reports a different deployment', () => {
        expect(buildAppUpdateStatusModel({
            platformOs: 'web', nativeUpdateUrl: null, webUi: { updateAvailable: true },
            desktop: { status: 'idle', availableVersion: null, error: null },
            ota: { isUpdatePending: false }, releaseNotes: { hasUnread: false }, changelog: { hasUnread: false }, t: fakeT,
        })).toMatchObject({ visible: true, kind: 'web-ui', actionLabel: 'Apply update' });
    });
