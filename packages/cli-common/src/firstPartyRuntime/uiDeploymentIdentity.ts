...
export function resolveUiDeploymentIdentity(params: Readonly<{
    ...
    if (!DIGEST_PATTERN.test(digest)) throw new Error('[relay-runtime] managed UI deployment digest is invalid');
    let previous: Record<string, unknown> | null = null;
    try {
        const parsed = JSON.parse(String(params.previousStateText ?? ''));
        previous = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as Record<string, unknown> : null;
    } catch {
        previous = null;
    }
    const previousDigest = String(previous?.uiDeploymentDigest ?? '');
    const previousId = String(previous?.uiDeploymentId ?? '');
    if (DIGEST_PATTERN.test(previousDigest) && previousDigest === digest && ID_PATTERN.test(previousId)) {
        return { digest, deploymentId: previousId };
    }
    const deploymentId = String(params.generateId() ?? '').trim();
    if (!ID_PATTERN.test(deploymentId)) throw new Error('[relay-runtime] generated UI deployment identity is invalid');
    ...
    async function visit(relativeDirectory: string): Promise<void> {
        ...
        for (const entry of entries) {
            ...
            if (entry.isFile()) {
                ...
                for await (const chunk of createReadStream(absolutePath)) fileDigest.update(chunk);
